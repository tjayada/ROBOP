#!/usr/bin/env python3
"""Render robot silhouettes from an evaluation JSON's poses (GT or estimated).

For every query, the mesh is FK-posed and rendered at the chosen pose
...
``--pose gt`` renders ``gt_pose`` (ground truth, model-independent); ``--pose
est`` renders the estimator's ``est_pose`` and marks ``pnp_failed`` (or missing
pose) frames as not-found. The est masks are the projection of each model's
prediction, directly comparable to the GT masks and to CNOS detections.

Baxter has no ``gt_pose`` (only an end-effector point), so ``--pose gt`` skips
it; ``--pose est`` works for every dataset.
...
def main():
    ...
    parser.add_argument("--pose", choices=("gt", "est"), default="gt",
                        help="which pose to project: gt_pose (ground truth) or est_pose "
                             "(the estimator's prediction, skipping pnp_failed frames)")
    ...
    print(f"Rendering {args.pose} masks for {len(remaining)} frames ({robot_name})")
    ...
    try:
        for image_id, q in enumerate(queries):
            ...
            # est_pose is absent on frames the estimator abstained on (pnp_failed);
            # those become not-found, exactly as a missing gt_pose does.
            pose = None if (args.pose == "est" and q.get("pnp_failed")) \
                else q.get(f"{args.pose}_pose")
    ...
    out = {
        "source": f"{args.pose}_mesh_projection",
        "results": results_path.name,
        "robot": robot_name,
    ...
    journal_path.unlink()
    print(f"Done. {n_rendered}/{len(entries)} {args.pose} masks -> {out_path}")
